...
| `aixodev-openhands-claude` | AIXO.Dev | **@jsmacmini** | content (9) | local-only · 44 commits | The **retired `openhands-claude` fork** (its @claude research line was merged back into `aixodev-openhands`, per `_projects/README`). 44 commits of research history, local-only — archive to preserve that history, or confirm fully-merged then delete. |
...
- **TastyPal** `_tastycal.com` _(identified — the **TastyCal** product-line under TastyPal; now written into the TastyPal + Sattvasic Health briefs)_ · `_tastypal.com` · `_tastypantry.com` · `_tastytrucks.app`
