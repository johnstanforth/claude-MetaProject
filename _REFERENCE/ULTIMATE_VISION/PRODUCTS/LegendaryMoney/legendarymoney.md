...
- **Product Line:** a standalone consumer **personal-finance-manager (PFM)** line — self-hosted, household-scoped, natural-language-first; "the anti-Quicken." *(No second Product Line is defined in the source files.)*
  ...
  - **Product: PriceScanner** *(working name — brand TBD).* A **comparison-shopping** app within LegendaryMoney: it parses purchases (incl. grocery receipts routed in via Divia.Network) and **price-matches each SKU across local stores** (e.g. Ralphs · Vons · Sprouts · Trader Joe's) **and online options** (e.g. Walmart+, Amazon Subscribe & Save) to surface the best price per item from nearby + online sources. *(New — surfaced 2026-06-21 in the cross-venture future-scenario reasoning, where the same routed receipt fans into LegendaryMoney **Accounts** AND **PriceScanner**; see [`../../../../_workflows/workflow_cross_venture_future_scenario.md`](../../../../_workflows/workflow_cross_venture_future_scenario.md). Whether it's a standalone Product, a second Product Line, or a feature of the PFM is **TBD**.)*
