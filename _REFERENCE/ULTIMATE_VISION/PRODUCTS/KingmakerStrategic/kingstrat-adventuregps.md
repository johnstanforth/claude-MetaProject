...
## Scope & framing — venture studio (~60–80%) vs. PE/VC (~20–40%)

KSVGPS's center of gravity is **not** the PE/VC CRM. Roughly **20–40%** of Kingmaker Strategic is the straightforward PE/VC side — a CRM + a "radar" that surfaces breakout projects (an OpenClaw at 2M downloads in week one) for small near-angel deals; useful, but no real complexity. The other **~60–80% is a venture studio**, and *that* is what makes KSVGPS hard and worth building: it models the place ideas live **before incorporation** — the equivalent of "inside the founder's head and early notebooks during the year before OpenClaw's first release" — across **a dozen-plus idea-stage ventures and dozens of people** John has worked with over 25+ years, each at its own maturity (some still nameless; some graduated to domains + branding; most still mostly in-head or shared with one or two would-be partners). This reframes the data model away from incorporation→dissolution toward **decade-scale idea-arcs** (vague idea → named → prototyped → small team → seed → PMF, sometimes → paused → rebooted-as-successor years later — the MobThought→CrowdMadness pattern), with **people/advisors as first-class** and **external-environment dependencies** (regulatory, data-licensing, platform) tracked by autonomous agents — the LLM economics that make many long-shelved ideas newly feasible. *(This framing drives the cross-venture workflows — see [`../../../../_workflows/README.md`](../../../../_workflows/README.md).)*
