...
- **Product Line:** TastyPal's consumer culinary line — an informal umbrella, not (yet) a single unified product. Holds the following Products / subsidiaries:
  ...
  - **Product: TastyCal** (subsidiary **"TastyCal (by TastyPal)"** — 🔵 **potential / under consideration**, not committed; **"TastyCal" is a working title that is now BLOCKED — `tastycal.com` is registered to a third party — pending rename + further research**). An **AI photo-based calorie / nutrition tracker** whose differentiator is pairing the AI food-photo view with a **Bluetooth-connected scale** that measures true portion *weight* (vs. the popular **Cal AI** app's photo-only estimation). The scale + AI-food-ID tech is **shared with — and white-labeled from — [Sattvasic Health](../SattvasicHealth/sattvasichealth.md)** (where John already planned the scale), re-branded for the TastyPal/culinary audience and sharpened by TastyPal food-database tie-ins (TastyPantry pantry inventory, taste preferences, recipes). **Direction note:** John's current lean is that the **primary home is the health-first scale-app inside Sattvasic Health** (esp. as a PBC under Divia.Foundation), with this **TastyPal white-label as a secondary, research-gated possibility** that itself needs a new name. The **product concept is unchanged**; only the name is blocked. Full product brief: [`tastycal.md`](tastycal.md); competitive research **R-007** (Cal AI).
...
- **TastyCal:** 🔵 **potential / under consideration**, **name BLOCKED** — no repo, no code. The **product concept** (a white-label of Sattvasic Health's Bluetooth-scale + AI-food-ID nutrition tech for the TastyPal audience) is unchanged, but the **working title "TastyCal" is unavailable** (`tastycal.com` registered to a third party), pending rename + further research. **Primary home leans to the health-first scale-app inside Sattvasic Health**; the TastyPal white-label is the secondary, research-gated path.
