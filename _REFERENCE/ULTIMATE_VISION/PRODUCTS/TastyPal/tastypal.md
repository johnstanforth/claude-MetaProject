...
- **Product Line:** TastyPal's consumer culinary line — an informal umbrella, not (yet) a single unified product. Holds the following Products / subsidiaries:
  ...
  - **Product: TastyCal** (subsidiary **"TastyCal (by TastyPal)"** — 🔵 **potential / under consideration**, not committed). An **AI photo-based calorie / nutrition tracker** whose differentiator is pairing the AI food-photo view with a **Bluetooth-connected scale** that measures true portion *weight* (vs. the popular **Cal AI** app's photo-only estimation). The scale + AI-food-ID tech is **shared with — and white-labeled from — [Sattvasic Health](../SattvasicHealth/sattvasichealth.md)** (where John already planned the scale), re-branded for the TastyPal/culinary audience and sharpened by TastyPal food-database tie-ins (TastyPantry pantry inventory, taste preferences, recipes). Recalled this session; intended domain `tastycal.com`. Full product brief: [`tastycal.md`](tastycal.md); competitive research **R-007** (Cal AI).
...
- **TastyCal:** 🔵 **potential / under consideration** — recalled this session; no repo, no code; a white-label of Sattvasic Health's Bluetooth-scale + AI-food-ID nutrition tech for the TastyPal audience.
