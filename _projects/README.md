...
| **kingstratvc-web** | KingmakerStrategic | _**Superseded** (parked) by `kingstrat-venturegps` 2026-06-14; kept for reference._ Private PKMS / “company intranet” for a PE/VC firm: portfolio & idea-stage startup tracking; a trial-run of a Divia.AI Enterprise client install. **Now a git fork of `diviahome-web`** (shares its history; client delta on `kingstrat-main`) | Python · Flask · SQLite→Postgres | Phase 00 pending — git fork of `diviahome-web` (+1 client commit) | Dual: AGPLv3 & Commercial ✅ (inherited) |
| **kingstrat-venturegps** | KingmakerStrategic | **KingStrat VentureGPS** — portfolio-tracking / venture-studio operating system for Kingmaker Strategic Venture Partners LLC (Studio Operations + LP Dashboard; VTL/GPS + air-traffic-control framing). Replaces `kingstratvc-web`; downstream client of `proto-divia_ai-enterprise` (`main` = pristine Enterprise-prototype mirror; `kingstrat-main` = client delta). App domain `kingstrat.ventures` | Python · Flask · SQLite→Postgres | Phase 00 pending — rebranded from a `kingstratvc-web` clone (2026-06-14) | Commercial/Proprietary ✅ |
...
- **proto-divia_ai-enterprise** — the **Python/Flask prototype of the Divia.AI Enterprise server** (the proprietary commercial team server in fast-iteration Python form; functionally the exact product the eventual Rust `divia_ai-enterprise` will be, only the language differing). A downstream fork of `diviahome-web` (the AGPL lab) — `diviahome-main` mirrors the lab, `main` carries the Enterprise delta — and itself the upstream of `kingstrat-venturegps`. Rebranded from a `diviahome-web` clone 2026-06-14. · `git@github.com:DiviaAI/proto-divia_ai-enterprise.git`
...
- **kingstrat-venturegps** (KingmakerStrategic) — **KingStrat VentureGPS**: the portfolio-tracking / venture-studio operating system for Kingmaker Strategic Venture Partners LLC (Studio Operations + LP Dashboard; VTL/GPS + air-traffic-control framing; app domain `kingstrat.ventures`). A downstream client of `proto-divia_ai-enterprise` (the Enterprise-server prototype): `main` = pristine mirror of the prototype, `kingstrat-main` = the VentureGPS client delta. Rebranded from a `kingstratvc-web` clone 2026-06-14. · `git@github.com:KingStratVC/kingstrat-venturegps.git`
- **kingstratvc-web** (KingmakerStrategic) — _**superseded** (parked) by `kingstrat-venturegps` on 2026-06-14; kept for reference._ Was a private PE/VC intranet trial-run forked from `diviahome-web`.
