#!/usr/bin/env python3
"""build_research_pdf.py (DEV_PLAN variant) — Stage 1 of the `research-pdf` skill.

ONE chaptered Markdown "book" (DEV_PLAN.md) -> ONE styled HTML file (branded cover
+ auto chapter-TOC + per-chapter sections), ready for Chrome headless --print-to-pdf.
Unlike the corpus variant (one file == one section), this splits a SINGLE document
on its top-level '## ' headings (fence-aware, so '#'-comments inside ```code``` are
never mistaken for headings) so each chapter/appendix becomes its own section with a
real TOC entry. Run with the markdown venv:

    /tmp/pdfvenv/bin/python /tmp/workgroups-devplan_build_research_pdf.py

See SKILL.md for the Chrome command and rationale. The CSS block below is copied
verbatim from the corpus build script so the three PDFs share one design system.
"""
import markdown, html as htmlmod, re, os

# ══ CONFIG — EDIT PER PROJECT ════════════════════════════════════════════════
KICKER    = "AIXO.Dev &middot; Workgroups"
TITLE     = "Building AIXO.Dev Workgroups"
SUBTITLE  = "A Developer&rsquo;s Build Guide &mdash; an O&rsquo;Reilly-style tutorial for the deterministic state server"
OUT_HTML  = "/tmp/workgroups-devplan-compendium.html"   # UNIQUE per project — avoid /tmp collisions!
SRC       = "/home/jstanforth/Code/AIXO.Dev/aixodev-workgroups/_specs_and_plans/_research/workgroups_foundations/DEV_PLAN.md"
RUNHEAD   = "AIXO.Dev Workgroups &middot; Developer Build Guide"   # constant per-page section label (running header)
COVER_META = (                                 # free HTML under the cover rule; <br> between lines
    "Phase 00 Foundations &middot; Build Guide<br>"
    "12 chapters + 3 appendices &middot; built from the foundations corpus<br>"
    "Follows the approved synthesis &middot; 2026-06-18"
)
COVER_NOTE = "A teaching artifact built entirely from the committed Phase-00 foundations corpus (synthesis + analysis-40/43 + the 45 base analyses). It includes concrete code sketches but edits no application source."
# ══ END CONFIG ═══════════════════════════════════════════════════════════════
